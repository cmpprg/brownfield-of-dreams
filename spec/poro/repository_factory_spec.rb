...
RSpec.describe RepositoryFactory, type: :model do
  before(:each) do
    ...
    @repos = RepositoryFactory.new(nil)
  end

  it "exists" do
    expect(@repos).to be_a(RepositoryFactory)
  end

  it "can return the created collection of repository objects" do
    allow_any_instance_of(RepositoryFactory).to receive(:repos_info).and_return(@info)
