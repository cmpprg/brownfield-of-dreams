...
RSpec.describe FollowersCollection, type: :model do
  before(:each) do
    ...
    @followers = FollowersCollection.new(nil)
  end

  it "exists" do
    expect(@followers).to be_a(FollowersCollection)
  end

  it "can return the created collection of follower objects" do
    allow_any_instance_of(FollowersCollection).to receive(:followers_info).and_return(@info)
